...
while True:
    ...
    if choiche=='1':
        ...
        (model,encoder,scaler,X_test_scaled,
         X_train_scaled,X_train, y_test)=creazione_modello(ratings_df,movies_df)
        # Raccomandazione per un utente
        recommended = recommend_movies(user_id=id_request, 
                                       ratings_df=ratings_df, 
                                       model=model, movies_df=movies_df, 
                                       encoder=encoder, scaler=scaler,X_train=X_train,num_recommendations=10)
        print(recommended)
        print('Credits:\nLombardi Alex\nGiacomo Vecchio\nCosimo Zaccaria\nMascia Mattia\nTHE END')
        break
    elif choiche=='2':
